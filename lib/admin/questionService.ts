...
export async function getQuestions(): Promise<FormattedQuestion[]> {
  const response = await apiClient.get<any>('/admin/questions/');
  ...
  const formattedQuestions: FormattedQuestion[] = questions.map((question: any, index: number) => ({
    id: question.question_id?.toString() || `question-${index}`,
    studentId: question.student_id || question.user_id || '',
    studentName: question.student_name || '알 수 없음',
    studentEmail: question.student_email || '',
    subject: question.subject_name || '미분류',
    problem: question.problem_content || '문제 없음',
    correctAnswer: question.correct_answer?.toString() || '정답 없음',
    explanation: question.explanation || '해설 없음',
    question: question.content || '질문 없음',
    answer: question.content_answer || undefined,
    status: question.state === 'answered' ? "completed" : "waiting",
    createdAt: question.created_at ? new Date(question.created_at).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
    imageUrl: question.image_url,
    source: question.source,
    examYear: question.exam_year,
    difficulty: question.difficulty,
    problemOptions: question.problem_options || [],
  }));
